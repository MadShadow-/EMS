-- author:RobbiTheFox,mcb		current maintainer:RobbiTheFox		v1.3
...
--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
function RandomMapGenerator.SetRandomSeed()
	
	local value = EMS.RD.Rules.RMG_Seed:GetValue() + 9876543210 + Game.RealTimeGetMs() + Game.RealTimeGetMs() * 100000
	
	while value >= 1000000000 do
...
function RandomMapGenerator.InitGenerationData()
	...
	if RandomMapGenerator.GenerationData.DebugMode then
		...
		for ID = 1, 16 do
			...
			Display.GfxSetSetFogParams(ID, 0.0, 1.0, 1, 58, 48, 38, 100000, 200000)
			Display.GfxSetSetLightParams(ID,  0.0, 1.0, 40, -15, -50,  180,160,140,  225,224,200)
			...
			if ID == 3 then
				Display.GfxSetSetSnowStatus(ID, 0.0, 1.0, 1)
			else
				Display.GfxSetSetSnowStatus(ID, 0.0, 1.0, 0)
			end
		end]]
		
		--gvWorldSize = 512 * 100
		--Logic.WorldGetSize = function()
			--return gvWorldSize
		--end
	...
	RandomMapGenerator.GenerationData.GenerateRivers		= EMS.RD.Rules.RMG_GenerateRivers:GetValue()
	...
	if XNetwork.Manager_DoesExist() == 1 then
		...
		-- just some testing in SP
		nplayers = 2
		players[1] = {id = 1, team = 1}
		players[2] = {id = 2, team = 2}
		players[3] = {id = 3, team = 1}
		players[4] = {id = 4, team = 2}
		players[5] = {id = 5, team = 1}
		players[6] = {id = 6, team = 2}
		players[7] = {id = 7, team = 1}
		players[8] = {id = 8, team = 2}
		nteams = 2
		teams[1] = 1
		teams[2] = 2
		--teams[3] = 3
	...
	if nteams <= 1 then
		RandomMapGenerator.GenerationData.GenerateRivers = false
	end
	...
	RandomMapGenerator.StructureSets = {
		ClayPit = {
			Entities = {{Type = Entities.XD_ClayPit1, Resource = cc1,},},
			Blocking = 11,
			TerrainHeights = {
				Area = 18,
		...
		StonePit = {
			Entities = {{Type = Entities.XD_StonePit1, Resource = sc1,},},
			Blocking = 11,
			TerrainHeights = {
				Area = 18,
		...
		IronPit = {
			Entities = {{Type = Entities.XD_IronPit1, Resource = ic1,},},
			Blocking = 11,
			TerrainHeights = {
				Area = 16,
				[-4]={[-4]= 0,[-3]=   0,[-2]=   -5},
		...
		SulfurPit = {
			Entities = {{Type = Entities.XD_SulfurPit1, Resource = rc1,},},
			Blocking = 11,
			TerrainHeights = {
				Area = 16,
		...
		NeutralVillageCenter = {
			Blocking = 10,
			Entities = {
				{Type = Entities.XD_VillageCenter, Player = 0,},
			},
			TerrainHeights = {
				Area = {x = 18, y = 18,},
			},
	...
	noiseMin = RandomMapGenerator.GenerationData.ThresholdLake
	noiseMax = RandomMapGenerator.GenerationData.ThresholdMountain
	...
	RandomMapGenerator.GenerationData.PlayerDistanceToMiddle = 0.667
	
	local rescls = 35 -- distance to fence
	local resmax = 90 -- highest distance
	local resfac = ((Logic.WorldGetSize() / 100) * RandomMapGenerator.GenerationData.PlayerDistanceToMiddle * math.cos(math.rad((180 - 360 / nplayers) / 2)) / 2 - rescls) / resmax
	local resoff = 5
	local resdist = {}
	
	-- vc
	for i = 1, 9 do
		table.insert(resdist, math.random(25 * resfac, 70 * resfac))
	end
	...
	for i = 1, vc do
		...
		local child = {
			Placement = {
				AreaMin = resdist[i] - resoff,
				AreaMax = resdist[i] + resoff,
		...
		if i == 1 then
			child.Childs = {{Data = {Entities = {{Type = Entities.PB_VillageCenter1}}}}}
		end
	...
	-- clay
	for i = 1, 9 do
		resdist[i] = math.random(50 * resfac, 70 * resfac)
	end
	...
	for i = 1, ca1 do
		...
		local child = {
			Placement = {
				AreaMin = resdist[i] - resoff,
				AreaMax = resdist[i] + resoff,
				Noise = RandomMapGenerator.GenerationData.ThresholdLake,
				NoiseMin = noiseMin,
				NoiseMax = noiseMax,
				Grid = 4,
			},
	...
	-- stone
	for i = 1, 9 do
		resdist[i] = math.random(60 * resfac, 80 * resfac)
	end
	...
	-- iron
	for i = 1, 9 do
		resdist[i] = math.random(70 * resfac, 90 * resfac)
	end
...
--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
function RandomMapGenerator.CopyDataToTable(_from, _to)
	...
	for k, v in pairs(_from) do
		...
		if type(v) == "table" then
			...
			RandomMapGenerator.CopyDataToTable(_from[k], _to[k])
		else
			_to[k] = _to[k] or v
		end
	end
...
function RandomMapGenerator.StartGenerateMap()
	...
	if _generationData.GenerateRivers == 2 then
		RandomMapGenerator.CreateFences(_generationData)
	elseif _generationData.GenerateRivers == 3 then
...
function RandomMapGenerator.FillPlayerLocationTable(_generationData)
	...
	for p = 1, nplayers do
		...
		if _generationData.RandomPlayerPosition then
			...
			x, y = RandomMapGenerator.FindBestPoint(_generationData, x, y, 20, 4)
		end
...
function RandomMapGenerator.Mirror(_generationData, _x, _y, _mapsize, _slize)
	...
	-- this edge will be covered by a team dividing River if GenerateRivers is true
	...
	if forcePlacement or node then
		...
		if _generationData.MirrorMap and RandomMapGenerator.StructGetCurrentPlayer(_generationData) > 0 then
			for p = 2, _generationData.NumberOfPlayers do
				...
				RandomMapGenerator.CreateStructure(_generationData, struct, mx, my, _generationData.Players[p].id)--, true) -- this works without doNotRegister since player 1 is set at last below
			end
		end
		
		-- if MirrorMap is true, this is player 1 and needs to be the last call to get the positioning for mirror sources right
		RandomMapGenerator.CreateStructure(_generationData, struct, x, y, _generationData.Players[RandomMapGenerator.StructGetCurrentPlayer(_generationData)].id)
	...
	if _struct.Data then
		...
		if data.Entities then
			for i = 1, table.getn(data.Entities) do
				...
				entity.x = _x * 100
				entity.y = _y * 100
				entity.p = data.Entities[i].Player or _player
...
function RandomMapGenerator.CreateEntities(_generationData)
	...
	for _,v in ipairs(_generationData.Entities) do
		RandomMapGenerator.CreateEntity(v)
	end
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------
function RandomMapGenerator.CreateEntity(_entity)
	...
	if entity.Name then
		Logic.SetEntityName(id, _struct.Name)
	end
	
	if entity.Health then
		SetHealth(id, _struct.Health)
	end
end
----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function RandomMapGenerator.Finalize(_generationData)
	...
	if EMS_CustomMapConfig.Peacetime > 0 and _generationData.GenerateRivers == 3 then
...
function RandomMapGenerator.Callback_OnPeacetimeEnded()
	...
	if RandomMapGenerator.GenerationData.GenerateRivers == 2 then
	
		for id in CEntityIterator.Iterator(CEntityIterator.OfTypeFilter(Entities.XD_WoodenFence15)) do
			DestroyEntity(id)
		end
		
	elseif RandomMapGenerator.GenerationData.GenerateRivers == 3 then
...
--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
 -- load this file at the end due to dependencies
 --++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
Script.Load("maps\\user\\EMS\\tools\\rmg_guil.lua")